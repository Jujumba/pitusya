...
use crate::ast::Ast;
use crate::lexer::tokens::{Token, OperatorType, BinaryOperator};
...
pub fn parse_expression_recurs(input: &mut InputFile) -> Result<Ast, String> {
    ...
    match next_token(input) {
        Token::Operator(OperatorType::Binary(op)) => Ok(
                Ast::BinaryNode {
                left: Box::new(ast), right: Box::new(match parse_expression_recurs(input) { // todo: recursion is extremely slooow!!!
                    e @ Err(_) => return e,
                    Ok(right) => right
                }), op
            }
